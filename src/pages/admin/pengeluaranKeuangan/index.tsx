...
import { validatePengeluaran } from "../../../validation/auth";
...
const PengeluaranKeuangan = () => {
  ...
  const [data, setData] = useState<PengeluaranProps>();
  const [showPhoto, setShowPhoto] = useState<boolean>(false);
  const [dataEdit, setDataEdit] = useState<PengeluaranProps>();
  ...
  const formik = useFormik({
    ...
    validationSchema: validatePengeluaran,
    onSubmit: (values) => {
  ...
  const editFormik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      axios
        ...
        .then(() => {
          toast.success("Berhasil Mengupdate Pengeluaran");
  ...
  const getDestroy = (id: string) => {
    toast.promise(
      ...
      {
        loading: "Menghapus...",
        success: "Berhasil Menghapus pengeluaran...",
        error: (error) => {
          ...
          <div className="overflow-x-auto px-10 pb-5">
            <table className="table">
              ...
              <tbody className="text-[#344767]">
                {data?.length > 0 ? (
                  data?.map((item: PengeluaranProps, index: number) => {
                    return (
